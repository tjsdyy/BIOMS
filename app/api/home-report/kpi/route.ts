...
import { getHomeKPIMetrics } from '@/lib/db/home-queries';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const metrics = await getHomeKPIMetrics({
      shop,
      salesperson,
    ...
    console.error('Error fetching home KPI metrics:', error);
    return NextResponse.json(
      { error: 'Failed to fetch home KPI metrics' },
      { status: 500 }
    );
  }
}
