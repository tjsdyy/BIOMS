...
export default function HomeFilterBar({ filters, onChange, user }: HomeFilterBarProps) {
  const [selectedQuickDate, setSelectedQuickDate] = useState<string>('lastYear');
  const apiClient = useMemo(() => createHomeApiClient(user), [user]);
  ...
  const { data: salespeopleData } = useQuery({
    queryKey: ['home-salespeople', filters.shop, filters.startDate, filters.endDate, user.id],
